...
then
	...
	mkdir $simulation_dir
	echo "Generate variants table"
	perl scripts/generate_variant_table.pl --reference $reference --num-genomes 10 --num-variants 1000 --random-seed 43 | sort -k 1,1 -k 2,2n > $variant_table
	echo "Generate reads. See $simulation_dir/generate_genomes.log for details"
	perl scripts/generate_genomes.pl --reference $reference --variants $variant_table --min-cov 30 --max-cov 60 --random-seed 43 --art-parameters '--noALN --len 250 --rndSeed 43 --paired --seqSys MS --sdev 50 --mflen 400' --out-dir $fastq_dir > $simulation_dir/generate_genomes.log
